...
def main():
    ...
    Width = 224
    Height = 224
    num_classes = 53  # Caltech101为102  cifar10为10
    pic_dir_out = 'out'
    sub_dir = '224_resnet50'
    pic_dir_data = 'ydata-yshoes-image-content-v2_0/fruits'
    ...
    (X_train, y_train) = get_data("shoes_data", 0.0, 0.7, data_format='channels_last', t='train')
    y_train = np_utils.to_categorical(y_train, num_classes)

    input_tensor = Input(shape=(224,224,3))
    base_model = ResNet50(input_tensor=input_tensor, include_top=True, weights='resnet50_weights_tf_dim_ordering_tf_kernels.h5')
    ...
    file_name = os.path.join(pic_dir_mine, 'resnet50_train_output1' + '.h5')
    ...
    print("---------")
    resnet50_train_output=resnet50_train_output.reshape(-1,1,1,2048)
    print(resnet50_train_output.shape)
    ...
    if os.path.exists(file_name):
        ...
        f.close()
    print('\nTesting ------------')  # 对测试集进行评估
    print(resnet50_test_output.shape)
    class_name_list = get_name_list(pic_dir_data)  # 获取top-N的每类的准确率
    resnet50_test_output=resnet50_test_output.reshape(-1,1,1,2048)
